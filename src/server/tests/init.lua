...
local archive = require(script.Parent.archive)
...
function tests.deconstruct_stockpile()
	...
	-- ensure no duplicate updated entities
	local updated_entities = {}

	for _, update in result.updates do
		if update.type == "entity_update" then
			if updated_entities[update.entity.id] then
				print(result.updates)
				error("duplicate updated entity: " .. update.entity.id)
			else
				updated_entities[update.entity.id] = true
			end
		end
	end

	assert(#(grid:query_entity { type = "stockpile" }) == 0, "stockpile is not destroyed")
	return grid
end
...
function run_tests()
	...
	for name, test in tests do
		...
		total += 1
		if success then
			print("Test", name, "passed in", DateTime.now().UnixTimestampMillis - test_t0, "ms")
			successes += 1
		else
			warn("Test", name, "failed:", err)
		end
	end
	print(
...
return {
	run_tests = run_tests,
	scout_attack_each_other = tests.scout_attack_each_other,
	deconstruct_stockpile = tests.deconstruct_stockpile,
	chatgpt_didnt_grift_me = tests.chatgpt_didnt_grift_me,
	extractor_filling_stockpile = tests.extractor_filling_stockpile,
}
