...
function tests.deconstruct_scaffold_with_autogenerated_wires()
	local world, teams = presets.blank_map()
	local scout_behavior = world.entity_configurations.scout
	world.global_configuration.construction_condition_enabled = false
	entity_mod.new_entity({
		type = "infinite_source",
		primary_coordinate = { 0, 0, 0 },
		owner = teams.team1.id,
...
function tests.necromancer()
	...
	local turret = entity_mod.new_entity({
		type = "turret",
		primary_coordinate = { 1, 0, -1 },
		owner = teams.team1.id,
	}, world)
	...
	action_phase_mod.run_action_phase(world)
	assert(scout.is_destroyed, "Scout should be destroyed by turret")
	local grave = world:query_entity({ type = "grave", coordinate = scout.primary_coordinate })[1]
	assert(grave ~= nil, "There should be a grave")
	local revived_entity = world.entities[grave.revives_into]
	assert(revived_entity ~= nil, "There should be a revived entity")

	for i = 1, world.entity_configurations.grave.build_time do
		action_phase_mod.run_action_phase(world)
	end
	local new_scout = world:query_entity({ type = "scout", coordinate = scout.primary_coordinate })[1]
	assert(new_scout ~= nil, "There should be a new scout")
	assert_eq(new_scout.health, 1, "New scout should have 1 health")
	assert(
		#world:query_entity { type = "vertex", coordinate = scout.primary_coordinate } == 1,
		"There should also be a vertex"
	)
end
