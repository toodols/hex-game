...
function remove_unsupported_vertices(grid: HexGrid, entity: Entity)
	if entity.status == "blueprint" then
		for _, coord in entity.coordinates do
			local vertices = grid:query_entity {
				coordinate = coord,
				type = "vertex",
				status = "blueprint",
				autogenerated = true,
			...
			if not supported then
				for _, vertex in vertices do
					server_entity_mod.remove_entity(grid, vertex)
				end
			end
		end
	end
	...
	for _, entry in data do
		...
		elseif entry.type == "deconstruct" then
			...
			if entity.status == "complete" or entity.status == "scaffold" then
				...
				remove_unsupported_vertices(grid, entity)
			end
		...
		if entity.type == "scout" then
			local extracted = util.table_extract(entity.queued_decisions, function(action)
				if action.type == "scout" then
					if
						not hex_grid_mod.line_of_sight(grid, entity.primary_coordinate, action.coordinate, entity.owner)
					then
						return true
					end
				end
